 ...
 * @author oldman96
 */
public class Modositas extends javax.swing.JFrame {
    ...
    public Modositas(Felszereles _f) {
        f=_f;
        initComponents();
        ComboboxFeltoltes();
    }
...
//            RaktariAzonositoTextfield.setText(sb.toString());
            ...
            KategoriaCombobox.setSelectedItem(f.getKategoria());
            AlkategoriaCombobox.setSelectedItem(f.getAlkategoria());
            RaktarhelyCombobox.setSelectedItem(f.getTelephely());
            AllapotSpinner.setValue(Integer.valueOf(f.getAllapot().replaceAll(".$", "")));
            EvjaratSpinner.setValue(Integer.valueOf(f.getEvjarat()));
            BeszerzArTextfield.setText(f.getBeszerzesiAr());
        ...
        try{
                     ...
                     && !SnTextfield.getText().equals("") && !RaktariAzonositoTextfield.getText().equals("") && !Allapot.equals("") && !Evjarat.equals(""))
            {
                ...
                stmt.executeUpdate("update raktartbl"
                    + " set "
                        + "Gyarto = (select gyartoktbl.ID from gyartoktbl where Gyarto like '"+GyartoTextfield.getText()+"'),"
                        + "Tipus = (select tipustbl.ID from tipustbl where Tipus like '"+TipusTextfield.getText()+"'),"
                        + "Kategoria = (select k.ID from kategoriatbl k join alkategoriatbl ak on k.AlkategoriaID = ak.ID where Kategoria like '"+KategoriaCombobox.getSelectedItem()+"'"
                                +" and Alkategoria like '"+AlkategoriaCombobox.getSelectedItem()+"'),"
                        + "Evjarat = (select "+ Evjarat +" ),"
                        + "BeszAr = (select "+ BeszerzArTextfield.getText() +" ),"
                        + "Telephely = (select telephelyektbl.ID from telephelyektbl where Telephely like '"+ RaktarhelyCombobox.getSelectedItem() +"'),"
                        + "Allapot = (select allapottbl.ID from allapottbl where Allapot like '"+ Allapot +"%'),"
                        + "Beszhely = (select '"+BeszerzHelyTextfield.getText()+"'),"
                        + "Gyariszam = (select '"+SnTextfield.getText()+"') "
                    + "where QRKod like '"+RaktariAzonositoTextfield.getText()+"';");
                Felszerelesek.setTm(new TableModel());
                Felszerelesek.refreshTable();
                JOptionPane.showMessageDialog(this, "Adatok módosítva.","Sikeres mentés!", JOptionPane.INFORMATION_MESSAGE, sikerIcon);
                this.dispose();
